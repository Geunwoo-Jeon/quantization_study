...
err = -0.02 / 5.26 = -0.0038
w2 보정: w2 -= (-0.0038)×(-4.74) → w2가 0.018 감소
loss 증가: 0.0000380
```
...
err = -0.02 / 1.10 = -0.0182
w2 보정: w2 -= (-0.0182)×(-0.33) → w2가 0.006 감소
...
err = -0.02 / 52.63 = -0.00038
w2 보정: w2 -= (-0.00038)×(-47.37) → w2가 0.018 감소
...
```
1) "이 오차가 얼마나 문제인가?" → H_inv[j,j] (자유도)로 판단
   err = (w - q) / H_inv[j, j]
   - 자유도가 크면 → err 작음 → 별 문제 아님
   - 자유도가 작으면 → err 큼 → 심각한 문제

2) "어디로, 얼마만큼 보상할 것인가?" → H_inv[j, k] (보상 강도/방향)으로 결정
   W[k] -= err × H_inv[j, k]
   - H_inv[j,k]의 크기 → 보상량 (클수록 많이 조정)
   - H_inv[j,k]의 부호 → 보상 방향 (음수면 반대 방향으로)
```

부호 규약 (w-q 사용):
- w < q (반올림으로 올라감) → w-q < 0 → err < 0
- 양의 상관 feature면 H_inv[j,k] < 0 (보상 방향이 반대)
- `W[k] -= (음수)×(음수) = W[k] -= 양수` → w_k 감소
...
def gptq_quantize(W, H, blocksize=128):
    ...
    for col_start in range(0, ch_in, blocksize):
        ...
        for j in range(col_end - col_start):
            ...
            W_block[:, j+1:] -= err.unsqueeze(1) * H_block[j, j+1:].unsqueeze(0)

        # 블록 간 보정: 이 블록에서 쌓인 오차를 나머지 열에 한꺼번에 전파
